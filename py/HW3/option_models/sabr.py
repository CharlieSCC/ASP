...
class ModelBsmMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp=1):
        ...
        for i in range(time_num):
            ...
            price_paths[t_row, :] = price_paths[i, :] * np.exp(sigma_paths[i, :] * np.sqrt(dt) * w_1 - 0.5 * sigma_paths[i, :] ** 2 * dt)
        self.price_paths = price_paths
        self.sigma_paths = sigma_paths
        self.prices_mc_output = np.fmax(price_paths[-1, :].reshape(1, -1) - strike.reshape(-1, 1), 0)
        self.prices = self.prices_mc_output.mean(axis=1)
...
class ModelNormalMC:
    ...
    def price(self, strike, spot, texp=None, sigma=None, cp=1):
        ...
        for i in range(time_num):
            ...
            w_1 = self.rho * z_1 + np.sqrt(1 - self.rho ** 2) * x_1
            sigma_paths[t_row, :] = sigma_paths[i, :] * np.exp(self.vov * np.sqrt(dt) * z_1 - 0.5 * self.vov ** 2 * dt)
            price_paths[t_row, :] = price_paths[i, :] + sigma_paths[i, :] * np.sqrt(dt) * w_1
        self.price_paths = price_paths
        self.sigma_paths = sigma_paths
        self.prices_mc_output = np.fmax(price_paths[-1, :].reshape(1, -1) - strike.reshape(-1, 1), 0)
        self.prices = self.prices_mc_output.mean(axis=1)
...
class ModelBsmCondMC:
    ...
    def price(self, strike, spot, texp=None, cp=1):
        ...
        self.prices = spot * np.exp(self.rho / self.vov * (sigma_paths[-1, :] - self.sigma) - self.rho ** 2 / 2 * self.i_t)
        self.sigma_bs = np.sqrt((1 - self.rho ** 2) * self.i_t / texp)
        self.prices_mc_output = self.bsm_formula(strike, self.prices, texp, self.sigma_bs)
        self.prices = np.mean(self.prices_mc_output, axis=1)
        return self.prices
...
class ModelNormalCondMC:
    ...
    def price(self, strike, spot, texp=None, cp=1):
        ...
        self.sigma_paths = sigma_paths
        self.i_t = np.sum(sigma_paths**2, axis=0)/(time_num+1)
        self.prices = spot + self.rho / self.vov * (sigma_paths[-1, :] - self.sigma)
        self.sigma_bs = np.sqrt((1 - self.rho ** 2) * self.i_t / texp)
        self.prices_mc_output = self.normal_formula(strike, self.prices, texp, self.sigma_bs)
        self.prices = np.mean(self.prices_mc_output, axis=1)
        return self.prices